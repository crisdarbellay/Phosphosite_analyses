...
def extract_contigs_from_sequence(reference_sequence, mov_pdb_file):
    # Extract residues from moving PDB file
    mov_residues = extract_residues_from_PDB(mov_pdb_file)

    # Define contigs based on reference sequence
    contigs = []
    start = 0
    while start < len(reference_sequence):
        # Find next mismatch between reference sequence and moving residues
        mismatch = None
        for i, (ref_aa, mov_aa) in enumerate(zip(reference_sequence[start:], mov_residues[start:])):
            if ref_aa != mov_aa:
                mismatch = i
                break
        if mismatch is None:
            # No mismatch found, add remaining sequence as a contig
            contigs.append(f"{reference_sequence[start:]}")
            break
        else:
            # Add sequence up to mismatch as a contig
            contigs.append(f"{reference_sequence[start:start + mismatch]}")
            start += mismatch

    # Add any remaining residues in moving PDB file as a contig
    if start < len(mov_residues):
        contigs.append(f"{mov_residues[start][0]}")

    return contigs

def compare_pdbs_by_sequence(ref_pdb_file, mov_pdb_file):
    # Extract residues from reference PDB file
    ref_residues = extract_residues_from_PDB(ref_pdb_file)
    mov_residues = extract_residues_from_PDB(mov_pdb_file)

    # Extract amino acid sequence from reference residues
    reference_sequence = "".join([res[0] for res in ref_residues])

    # Extract contigs from moving PDB file based on reference sequence
    contigs = extract_contigs_from_sequence(reference_sequence, mov_pdb_file)

    # Extract residues corresponding to each contig in moving PDB file
    mov_residues_by_contig = [extract_contig_from_residue_table(mov_residues, contig)[0] for contig in contigs]

    # Index residues in moving PDB file based on contigs
    all_matching_positions_resIDs = []
    for contig_residues in mov_residues_by_contig:
        all_matching_positions_resIDs.append(index_contigs_in_generated_sequence(contig_residues, [contig_residues[0][0]])[2])

    # Combine indexed residues into a table similar to reference table
    mov_table = []
    for contig_residues, positions in zip(mov_residues_by_contig, all_matching_positions_resIDs):
        for residue, position in zip(contig_residues, positions):
            mov_table.append(residue + (position,))
...
def index_contigs_in_generated_sequence(residue_table, contigs_as_list_of_strings):
    ...
    for string in contigs_as_list_of_strings:
        ...
        while True:
            ...
            matching_positions = range(position, position + len(string))  # Fixed indexing here
            all_matching_positions.extend(matching_positions)
            for i_position in matching_positions:  # Iterate over matching positions
...
def rigid_alignment(ref_file, mov_file, ref_aa_names, mov_aa_names):
    ...
    print("Dimensions de ref_centered_coords :", ref_centered_coords.shape)
    print("Dimensions de mov_centered_coords :", mov_centered_coords.shape)
...
def CompareTwoPDBs(ref_pdb_file, mov_pdb_file):
    # Extract residues from reference PDB file
    mov_residues = extract_residues_from_PDB(mov_pdb_file)
    ref_residues = extract_residues_from_PDB(ref_pdb_file)
    ...
    ref_coords, ref_residue_ids = parse_pdb_file(ref_pdb_file)
    mov_coords, mov_residue_ids = parse_pdb_file(mov_pdb_file)
    ref_aa_names = [res[0] for res in ref_residues]
    mov_aa_names = [res[0] for res in mov_table]
    rotated_mov_coords, rmsd = rigid_alignment(ref_pdb_file, mov_pdb_file, ref_aa_names, mov_aa_names)

    # Update the coordinates in the mobile PDB file
    update_pdb_coordinates(mov_pdb_file, mov_pdb_file.split('.pdb')[0] + '_sorted-aligned.pdb', rotated_mov_coords)
