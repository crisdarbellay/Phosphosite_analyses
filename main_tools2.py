import numpy as np
from loop import extract_sequence_from_pdb
...
def align_and_find_matching_subsequences(sequence1, sequence2):
    matching_subsequences1 = []
    matching_subsequences2 = []
    ...
    for j in range(i + 5, len(sequence1) + 1):
        ...
        if subsequence in sequence2 and j!=len(sequence1):
...
def define_contigs(pdb1, pdb2):
    sequence1 = extract_sequence_from_pdb(pdb1)
    sequence2 = extract_sequence_from_pdb(pdb2)
