...
def save_kinase_dictionary_to_txt(kinase_data, output_file):
    with open(output_file, 'w') as txt_file:
        for kinase, gene_data in kinase_data.items():
            ...
            txt_file.write(f"Kinase animal: {gene_data['kinase_animal']}\n")
            txt_file.write(f"Nombre de sites: {num_sites}\n\n")
            for gene, substrates in gene_data.items():
                if gene != 'kinase_animal':
                    txt_file.write(f"Gene: {gene}\n")
                    for substrate in substrates:
                        txt_file.write(f"Acc ID: {substrate['acc_id']}\n")
                        txt_file.write(f"Substrate: {substrate['substrate']}\n")
                        txt_file.write(f"Substrate gene ID: {substrate['substrate_gene_id']}\n")
                        txt_file.write(f"Site group ID: {substrate['site_group_id']}\n")
                        txt_file.write(f"Site aa: {substrate['site_aa']}\n")
                        txt_file.write(f"Domain: {substrate['domain']}\n")
                        txt_file.write(f"In vivo: {substrate['in_vivo']}\n")
                        txt_file.write(f"In vitro: {substrate['in_vitro']}\n")
                        txt_file.write(f"Filename: {substrate['filename']}\n\n")


def create_kinase_dictionary_2_substrates(file_path):
    kinase_data = {}
...
def create_kinases_datas(file_path,secondary_structures,human_cif_directory,mouse_cif_directory,rat_cif_directory,output_directory):
    ...
    # Separate group in_vivo and in_vitro
        ...
        with open(output_file, 'w') as out_file:
            header = "Gene\tRes\tScore\tpercent\talpha\tbeta\tiso_B\talpha3\talphaI\thydturn\tAFConf\tNextAA\torga\tsite_aa\tsub_id\tin_vivo\tin_vitro\n"
